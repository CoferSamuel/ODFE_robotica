...
{
    // Static RNG: seeded once, reused across calls (was: re-seeding from hardware entropy every call)
...
//    auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
    ...
    // Webots stores images as BGRA. Convert to BGR in one OpenCV call instead of per-pixel loop.
    const unsigned char* webotsImageData = _camera->getImage();
    cv::Mat bgraFrame(newImage.height, newImage.width, CV_8UC4, (void*)webotsImageData);
    cv::Mat bgrFrame;
    cv::cvtColor(bgraFrame, bgrFrame, cv::COLOR_BGRA2BGR);
    std::vector<unsigned char> rgbImage(bgrFrame.data, bgrFrame.data + bgrFrame.total() * bgrFrame.elemSize());

    // Asignar la imagen BGR al tipo TImage de Robocomp
...
void SpecificWorker::receiving_depthImageData(webots::RangeFinder* _rangeFinder, double timestamp)
{
    ...
    // Scale and copy depth buffer in two vectorised passes instead of a byte-by-byte loop
    std::vector<float> scaledDepth(imageElementCount);
    std::transform(webotsDepthData, webotsDepthData + imageElementCount,
                   scaledDepth.begin(), [](float v){ return v * 10.0f; });
    newDepthImage.depth.resize(imageElementCount * sizeof(float));
    std::memcpy(newDepthImage.depth.data(), scaledDepth.data(), newDepthImage.depth.size());
...
RoboCompLidar3D::TData SpecificWorker::Lidar3D_getColorCloudData()
{
...
void SpecificWorker::OmniRobot_setSpeedBase(float advx, float advz, float rot)
{
    ...
    // printf removed — was printing on every speed command, adding unnecessary I/O in the hot path
    for (int i = 0; i < 4; i++)
    {
...
void SpecificWorker::parseHumanObjects() {
    // Cache the CROWD node pointer — avoid scene-graph lookup (getFromDef) every compute cycle
    if (!crowdNode)
        crowdNode = robot->getFromDef("CROWD");
