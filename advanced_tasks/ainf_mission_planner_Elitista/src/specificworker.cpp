...
void SpecificWorker::initialize() {
  ...
  connect(viewer, &AbstractGraphicViewer::new_mouse_coordinates, this,
          &SpecificWorker::slot_new_target);
  ...
  connect(pushButton_startFollow, &QPushButton::clicked, this, [this]() {
    ...
    try {
      ...
      qInfo() << "Path following started toward" << last_target.x
              << last_target.y;
  ...
  try {
    ...
    auto draw_polyline = [this](const RoboCompNavigator::TLayout &polyline,
                                const QPen &pen) {
      if (polyline.size() < 2)
        return;
    ...
    for (size_t i = 0; i < map_data.objects.size(); ++i) {
      ...
      // Guardar el nombre en la lista de pendientes para la máquina de estados
      if (!obj.name.empty()) {
        pending_objects.push_back(obj.name);
      }

      const QColor object_color =
          QColor::fromHsv(static_cast<int>((i * 47) % 360), 210, 230);
      ...
      if (obj.layout.size() >= 3) {
        ...
        viewer->scene.addPolygon(polygon, QPen(object_color, 0.07),
    ...
    if (not map_data.layout.empty()) {
      ...
      auto *legend = viewer->scene.addText(
          "Room: magenta (thick) | Objects: unique filled colors");
    ...
    // Iniciar misión automáticamente si se encontraron objetos
    if (!pending_objects.empty()) {
      mission_active = true;
      qInfo() << "Misión auto-iniciada con" << pending_objects.size()
              << "objetos por visitar.";
    }
  } catch (const Ice::Exception &e) {
...
void SpecificWorker::compute() {
  ...
  try {
    ...
    qWarning() << "Error requesting robot pose from Navigator:" << e.what();
  }
  try {
    ...
    if (not status.statusMessage.empty()) {
      state_text +=
          QString(" (%1)").arg(QString::fromStdString(status.statusMessage));
    }
    label_missionStatusValue->setText(state_text);

    label_distanceToTargetValue->setText(
        QString::number(status.distanceToTarget, 'f', 2));
    label_etaValue->setText(QString::number(status.estimatedTime, 'f', 2));
    label_currentSpeedValue->setText(
        QString::number(status.currentSpeed, 'f', 2));

    // --- MÁQUINA DE ESTADOS PARA VISITAR OBJETOS ---
    if (mission_active) {
      // Si el robot está parado o acaba de llegar al destino
      if (status.state == RoboCompNavigator::NavigationState::IDLE ||
          status.state == RoboCompNavigator::NavigationState::REACHED) {
        if (!pending_objects.empty()) {
          // Extraemos el siguiente objeto de la lista
          std::string target_obj = pending_objects.front();
          pending_objects.erase(pending_objects.begin());

          qInfo() << ">>> Siguiente destino: yendo hacia el objeto"
                  << QString::fromStdString(target_obj);
          navigator_proxy->gotoObject(target_obj);
        } else {
          qInfo() << ">>> ¡Todos los objetos han sido visitados exitosamente!";
          mission_active = false;
        }
      }
      // Si hubo un error en la navegación o está bloqueado
      else if (status.state == RoboCompNavigator::NavigationState::ERROR ||
               status.state == RoboCompNavigator::NavigationState::BLOCKED) {
        qWarning() << ">>> Error navegando hacia el objeto, abortando la "
                      "misión secuencial.";
        mission_active = false;
      }
    }
...
void SpecificWorker::slot_new_target(QPointF target) {
  ...
  try {
    ...
    RoboCompNavigator::TPoint destination{static_cast<float>(target.x()),
...
void SpecificWorker::redraw_planned_path(
    const RoboCompNavigator::TPoint &current_source) {
  if (viewer == nullptr)
    return;
  ...
  auto *first_segment = viewer->scene.addLine(
      QLineF(p1.x, p1.y, p2_first.x, p2_first.y), path_pen);
  ...
  for (size_t i = 1; i < planned_path_points.size() - 1; ++i) {
    ...
    auto *segment = viewer->scene.addLine(
        QLineF(p1_mid.x, p1_mid.y, p2_mid.x, p2_mid.y), path_pen);
    segment->setZValue(200);
    planned_path_items.push_back(segment);
  }

  auto *start_marker =
      viewer->scene.addEllipse(current_source.x - 0.08, current_source.y - 0.08,
                               0.16, 0.16, QPen(QColor("darkOrange"), 0.03),
  ...
  auto *goal_marker = viewer->scene.addEllipse(
      goal.x - 0.09, goal.y - 0.09, 0.18, 0.18, QPen(QColor("red"), 0.03),
...
// RoboCompNavigator::Result this->navigator_proxy->getPath(TPoint source,
// TPoint target, float safety) RoboCompNavigator::TPoint
// this->navigator_proxy->gotoObject(string object) RoboCompNavigator::TPoint
// this->navigator_proxy->gotoPoint(TPoint target) RoboCompNavigator::void
// this->navigator_proxy->resume() RoboCompNavigator::void
// this->navigator_proxy->stop()
