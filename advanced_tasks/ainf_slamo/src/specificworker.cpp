...
void SpecificWorker::compute()
{
    ...
    if (++timing_counter >= 40)
    {
        ...
        std::cout << std::format("[TIMING] Viz: {:.1f} ms | MPPI: {:.1f} ms | Cycle: {:.1f} ms ({}%) | Lidar+Loc+other: ~{}%",
                     viz_ms, mppi_ms, cycle_ms,
                     static_cast<int>(cycle_pct),
                     static_cast<int>(cpu_total - cycle_pct)) << std::endl;
    }
}
...
void SpecificWorker::read_lidar()
{
    ...
    while(!stop_lidar_thread)
    {
        ...
        try
        {
            ...
            static bool gt_pose_supported = true;
            if (params.USE_WEBOTS && gt_pose_supported)
            {
                try
                {
                ...
                {
                    qWarning() << "[read_lidar] Webots bridge does not support getObjectPose(). GT disabled.";
                    gt_pose_supported = false; // Disable to prevent spamming exceptions
                }
                catch (const Ice::Exception &e)
                {
                    qWarning() << "[read_lidar] Exception getting GT pose:" << e.what();
                }
            }
