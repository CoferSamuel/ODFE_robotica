...
#include <algorithm>
#include <cmath>
#include <chrono>   // For visualization throttle (steady_clock)
...
void GridESDF::update_visualization(bool show_inflation)
{
    ...
    // ── Throttle: rebuild Qt scene at most 2Hz ────────────────────────────
    // Rebuilding 6 inflation layers at 20Hz consumes significant CPU for
    // purely cosmetic updates. 2Hz (500ms) is imperceptible to the eye.
    static auto last_viz_rebuild = std::chrono::steady_clock::now();
    const auto now_viz = std::chrono::steady_clock::now();
    const auto ms_since_rebuild = std::chrono::duration_cast<std::chrono::milliseconds>(
        now_viz - last_viz_rebuild).count();
    if (ms_since_rebuild < 500 && !inflation_tiles_.empty())
        return;
    last_viz_rebuild = now_viz;
