...
#include <omp.h>      // OpenMP parallel trajectory rollouts

// Per-thread RNG — each OpenMP thread gets its own seeded generator
// so we avoid both data races and serialization on rng_.
static thread_local std::mt19937 tl_rng{std::random_device{}()};
static thread_local std::normal_distribution<float> tl_dist{0.0f, 1.0f};
    ...
    const std::vector<float>& pose_cov)
{
    ...
    // Pre-compute path squared distances for faster nearest point lookup
    ...
    // ========================================================================
    // ── Parallel sampling ─────────────────────────────────────────────────
    // Each trajectory is independent: no shared writes except into
    // pre-allocated per-k slots. The thread_local RNGs avoid contention.
    #pragma omp parallel for schedule(dynamic, 4) default(none) \
        shared(all_controls, all_trajectories, costs, cost_breakdowns, \
               path, obs_to_use, esdf, noise_buffer_vx_, noise_buffer_vy_, \
               noise_buffer_omega_, prev_controls_, params_, path_size, \
               cos_theta_init, sin_theta_init, current_state, robot_radius_sq, \
               Sigma_pose)
    for (int k = 0; k < K; ++k)
    {
        ...
        int closest_path_idx = 0;  // per-trajectory path progress cursor (private per thread)
        ...
        for (int t = 0; t < T && !collision; ++t)
        {
            ...
            // ==============================================================
            // PATH FOLLOWING COST — sliding-window lookahead O(window) not O(N)
            // closest_path_idx is a local int, private to this trajectory.
            // ==============================================================
            {
                const int search_start = std::max(0, closest_path_idx - 2);
                const int search_end   = std::min(path_size, closest_path_idx + 12);
                float path_min_dist_sq = std::numeric_limits<float>::max();
                for (int pi = search_start; pi < search_end; ++pi)
                {
                    float dx = state.x - path[pi].x();
                    float dy = state.y - path[pi].y();
                    float d_sq = dx * dx + dy * dy;
                    if (d_sq < path_min_dist_sq)
                    {
                        path_min_dist_sq = d_sq;
                        closest_path_idx = pi;
                    }
                }
