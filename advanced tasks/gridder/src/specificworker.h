...
// If you want to reduce the period automatically due to lack of use, you must
...
/**
 * \brief Class SpecificWorker implements the core functionality of the
 * component.
 */
...
public:
   ...
   */
  SpecificWorker(const ConfigLoader &configLoader, TuplePrx tprx,
                 bool startup_check);
  ...
  /**
   * @brief Checks if there is a line of sight from source to target point
   * without any obstacle in the grid, considering the robot radius
   * @param source
   * @param target
   * @param robotRadius
   * @return
   */
  bool Gridder_LineOfSightToTarget(RoboCompGridder::TPoint source,
                                   RoboCompGridder::TPoint target,
                                   float robotRadius);

  /**
   * @brief Finds the closest free point in the grid to the given source point,
   * if the source point is blocked by an obstacle
   * @param source
   * @return
   */
  RoboCompGridder::TPoint
  Gridder_getClosestFreePoint(RoboCompGridder::TPoint source);
   ...
   * @param tryClosestFreePoint If true, finds closest free point when target is
   * blocked
   * @param targetIsHuman Whether the target is a human (affects pathfinding
   * behavior)
   * @param safetyFactor 0.0=shortest path, 1.0=safest path (prefer center of
   * free space)
  ...
  /**
   * @brieg Sets the location of the robot and the target, updates the grid with
   * the free and obstacle points, computes paths from source to target and
   * returns them
   * @param source
   * @param target
   * @param freePoints
  ...
  Gridder_setLocationAndGetPath(RoboCompGridder::TPoint source,
                                RoboCompGridder::TPoint target,
                                RoboCompGridder::TPointVector freePoints,
                                RoboCompGridder::TPointVector obstaclePoints);

  /**
   * @ brief Returns the current pose of the robot (position and orientation
   * with covariance if available)
   * @return
   */
  ...
  bool Gridder_setTargetWithOptions(RoboCompGridder::TPoint target,
                                    RoboCompGridder::NavigationOptions options);
...
private:
  ...
  bool cancel_from_mouse =
      false; // cancel current target from mouse right click
  bool external_map_loaded =
      false; // true if an external map was loaded (disables dynamic updates)
  ...
  struct Params {
    ...
    float LOCALIZER_MIN_TRANS_DIFFUSION =
        50.f; // mm per cycle (~1000mm/s at 20Hz)
    ...
    int MAP_DILATION_RADIUS =
        1; // Dilate obstacles by N cells to fill gaps (0 = disabled)
  ...
  // Double buffer for estimated robot pose (written by localizer thread, read
  // by main thread)
  ...
  std::atomic<bool> map_ready_for_localization{
      false}; // Set after map is fully loaded
  ...
  // Draw
  void draw_paths(const std::vector<std::vector<Eigen::Vector2f>> &paths,
                  QGraphicsScene *scene, bool erase_only = false);
  void draw_path(const std::vector<Eigen::Vector2f> &path,
                 QGraphicsScene *scene, bool erase_only = false);
  void draw_lidar_points(const std::vector<Eigen::Vector2f> &points_local,
  ...
  std::optional<Eigen::Affine2f>
  get_current_pose(const Eigen::Affine2f &gt_pose, std::uint64_t timestamp);
  ...
  // Double buffer for MPPI control output (written by MPPI thread, read by main
  // thread) Stores (vx, vy, omega, valid) - valid indicates if the command is
  // fresh
  struct MPPIOutput {
    float vx = 0.f;
    float vy = 0.f;
  ...
  void
  draw_mppi_trajectory(const std::vector<MPPIController::State> &trajectory);
  std::mutex mutex_mppi_trajectory; // Protects trajectory visualization data
  std::vector<MPPIController::State>
      last_optimal_trajectory; // For visualization from main thread
  ...
  static constexpr float CPU_AVG_ALPHA =
      0.1f; // Smoothing factor (0.1 = slow, 0.5 = fast)
