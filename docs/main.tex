...
\textbf{TODO (Partner's Task):} Implement AI-based room identification by reading wall numbers to determine the current room.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\columnwidth]{multimedia/task4/twoRoomtwoDoors_map.png}
    \caption{Simulation environment: Two rooms connected by two doors, with red and green badges on opposite walls for orientation reference.}
    \label{fig:environment_overview}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Coarse Localization (Grid Search):} Tests 200 candidate poses across the room:
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Fine Localization (Incremental SVD):} After bootstrap, the standard pose correction takes over, refining the position each control cycle.
\end{enumerate}
...
\subsubsection{Two-Phase Convergence}
...
\subsubsection{Grid Search Mathematics}
...
\subsubsection{Incremental Pose Correction}
...
\begin{itemize}
    \item \textbf{ROOM Nodes}: Represent physical rooms (e.g., Room 0, Room 1). Store metadata including dimensions, entry heading, and the ID of the door used to enter.
    \item \textbf{DOOR Nodes}: Represent physical connections between rooms. Store geometric endpoints ($p_1$, $p_2$) and their center position.
\end{itemize}

\subsubsection{Dynamic Construction}
\begin{enumerate}
...
\subsubsection{Topological Linking}
...
\subsubsection{Exploration State}
...
\subsubsection{Entry Door Identification}
...
\subsubsection{Graph-Based Door Selection (Priority System)}
The \texttt{select\_door\_from\_graph()} function implements a priority-based algorithm to choose the next target door:

\begin{enumerate}
    \item \textbf{Priority 1 -- Unexplored, Non-Entry Doors}: The system first searches for doors that are both unexplored (fewer than 2 connections in the graph) AND not the entry door. This prioritizes exploration of new areas while avoiding immediate backtracking.
    
    \item \textbf{Priority 2 -- Any Unexplored Door}: If no non-entry unexplored doors exist, the system accepts any unexplored door, even if it is the entry door. This ensures exploration continues.
    
    \item \textbf{Priority 3 -- Traversal Logic}: When all doors are explored:
    \begin{itemize}
    ...
    \item \textbf{Priority 4 -- Backtrack}: As a last resort (when no other options exist), the robot backtracks through the entry door.
\end{enumerate}

\subsubsection{Sticky World-Coordinate Tracking}
...
\subsubsection{Door Visualization}
...
\subsubsection{GUI Reorganization}
...
\textbf{TODO:} This section will describe the AI-based room identification system that reads wall numbers to determine the current room. This functionality is currently under development by a team partner.

\subsection{Conclusions}
\textbf{TODO:} This section will summarize the achievements of Activity 4, discuss the effectiveness of the multi-room navigation system, and outline potential future improvements.
